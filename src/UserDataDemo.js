...
import { setupUserData, useUserData } from "./helpers/userdata";
...
function SignOut() {
  ...
  return (
    <div id="container">
      <h6>
        {user?.email} is a {userdata?.gender}
      </h6>
  ...
  const proceed = () => {
    ...
    if (isNewUser) {
      createUserWithEmailAndPassword(auth, email, password)
        .then(() => setupUserData({ gender }))
        .catch(handleError(setErrorText));
    } else {
      signInWithEmailAndPassword(auth, email, password).catch(
        handleError(setErrorText)
      );
    }
  };
    ...
    <div id="container">
      ...
      {isNewUser ? (
        <h6>
          ...
          <button className="btn-flat" onClick={toggleNewUser}>
            Login
          </button>
        </h6>
      ) : (
      ...
      {isNewUser ? (
        <input
          onChange={trackChange(setConfirmPassword)}
